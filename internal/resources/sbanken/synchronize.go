...
func (resource *Resource) Synchronize() error {
	sub := resource.sub
	...
	accounts, err := resource.GetAccounts()
	if err != nil {
		return fmt.Errorf("failed to get sbanken accounts: %w", err)
	}
	...
	for err := range errors {
		return fmt.Errorf("failed to synchronize some accounts, sample error: %w", err)
	}
